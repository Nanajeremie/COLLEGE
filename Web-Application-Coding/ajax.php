...
if (isset($_POST['submit'])) {
    ...
    if ($filiere != 'Select') {
        ...
        if (is_object($checkClass) == false) {
            $insertClass = $obj->Insert('classe', ['ID_FILIERE', 'NOM_CLASSE', 'MONTANT_SCOLARITE'], [$filiere, $classe_nom, $class_fees]);
...
if (isset($_POST['upd'])) {
    ...
    $selectClass = $obj->Requete("SELECT * FROM classe c, filieres f WHERE c.ID_FILIERE = f.ID_FILIERE AND c.ID_CLASSE='" . $matricule . "'");
    $selectFillier = $obj->Select('filieres', [], [], $orderBy = '', $order = 1);
        ...
        if ($getSelect = $selectClass->fetch()) {
            $string .= '<div class="col-lg-4">
                   ...
                   <div class="input-group">
                       ...
                       <option value="' . $getSelect['ID_FILIERE'] . '">' . $getSelect['NOM_FILIERE'] . '</option>';


            if (is_object($selectFillier)) {
                while ($repListe = $selectFillier->fetch()) {

                    $string .= "<option value='" . $repListe['ID_FILIERE'] . "'>" . $repListe['NOM_FILIERE'] . "</option>";
                }
            }
            $string .= '</select>
                       ...
                       <div class="col-lg-4">
                           <label for="up_classe_nom">Class Name</label>
                           <div class="input-group">
                               <input type="text" class="form-control" name="classe_nom" id="up_classe_nom" placeholder="Enter Class Name" oninput="EnableDecimal(this)" value="' . $getSelect['NOM_CLASSE'] . '" required>
                       ...
                       <div class="col-lg-4">
                           <label for="up_class_fees">School Fees</label>
                           <div class="input-group">
                               <input type="number" class="form-control" name="class_fees" id="up_class_fees" placeholder="Enter Class Name" oninput="EnableDecimal(this)" value="' . $getSelect['MONTANT_SCOLARITE'] . '" required>
                       ...
                       <div class="col-lg-12 text-center my-5">
                           <input class="btn btn-primary w-50" type="button" name="submit_class" id="update" value="Update class" onclick="Update()"> 
                       </div> </div>';
        ...
        if (is_object($checkClass) == true) {
            if ($checkClass->rowCount() == 1) {
                ...
                if ($getIdClasse != $idClasse) {
                    ...
                    if ($up_filiere != $getFillieres) {
                        echo "exit";
                    } else {
                        $updateClass1 = $obj->Update('classe', ['ID_FILIERE', 'NOM_CLASSE', 'MONTANT_SCOLARITE'], [$up_filiere, $up_classe_nom, $up_class_fees], array('ID_CLASSE' => $idClasse));
                        if ($updateClass1 == 1) {
                            echo "okk";
                        }
                    }
            ...
            $updateClass2 = $obj->Update('classe', ['ID_FILIERE', 'NOM_CLASSE', 'MONTANT_SCOLARITE'], [$up_filiere, $up_classe_nom, $up_class_fees], array('ID_CLASSE' => $idClasse));
            if ($updateClass2 == 1) {
                echo "ok";
            }
        }
